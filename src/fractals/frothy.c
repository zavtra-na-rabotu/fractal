...
void	init_frothy_area(t_package *pack)
{
	...
	pack->fractal_area.mouse_pos.r = 1;
	pack->fractal_area.mouse_pos.i = 1.02871;
	palette1(pack->palette);
}

void	frothy_iteration(int x, int y, t_package *pack)
{
	t_c_num		z;
	t_c_num		c;
	t_c_num		tmp;
	...
	{
		tmp.r = (z.r * z.r) - (z.i * z.i) - c.r * (z.r + z.i);
		tmp.i = 2 * z.r * z.i - c.i * (z.r - z.i);
...
void	*draw_frothy_part(void *arg)
{
	t_thread_data	*data;
	int				x;
	int				y;
	...
	while (y < HEIGHT)
	{
		...
		while (x < data->x_end)
		{
			frothy_iteration(x, y, data->pack);
			x++;
		}
		y++;
	}
...
void	draw_frothy(t_package *pack)
{
	...
	while (i < MAX_THREADS)
	{
		...
		pthread_create(&threads[i], NULL, draw_frothy_part, &data[i]);
		i++;
	}
	while (i--)
