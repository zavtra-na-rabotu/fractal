...
void	spider_iteration(int x, int y, t_package *pack)
{
	t_c_num		z;
	t_c_num		c;
	t_c_num		tmp;
...
void	*draw_spider_part(void *arg)
{
	t_thread_data	*data;
	int				x;
	int				y;
	...
	while (y < HEIGHT)
	{
		...
		while (x < data->x_end)
		{
			spider_iteration(x, y, data->pack);
			x++;
		}
		y++;
	}
...
void	draw_spider(t_package *pack)
{
	...
	while (i < MAX_THREADS)
	{
		...
		pthread_create(&threads[i], NULL, draw_spider_part, &data[i]);
		i++;
	}
	while (i--)
