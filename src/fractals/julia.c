...
void	init_julia_area(t_package *pack)
{
	...
	pack->fractal_area.mouse_pos.r = -0.3;
	pack->fractal_area.mouse_pos.i = 0.69;
	palette2(pack->palette);
}

void	julia_iteration(int x, int y, t_package *pack)
{
	t_c_num		z;
	t_c_num		c;
	t_c_num		tmp;
...
void	draw_julia(t_package *pack)
{
	...
	while (i < MAX_THREADS)
	{
		...
		pthread_create(&threads[i], NULL, draw_julia_part, &data[i]);
		i++;
	}
	while (i--)
