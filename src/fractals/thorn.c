...
void	init_thorn_area(t_package *pack)
{
	...
	pack->fractal_area.mouse_pos.i = 1;
	palette1(pack->palette);
}

void	thorn_iteration(int x, int y, t_package *pack)
{
	t_c_num		z;
	t_c_num		c;
	t_c_num		tmp;
	...
	{
		tmp.r = z.r / cos(z.i) + c.r;
		tmp.i = z.i / cos(z.r) + c.i;
...
void	draw_thorn(t_package *pack)
{
	...
	while (i < MAX_THREADS)
	{
		...
		pthread_create(&threads[i], NULL, draw_thorn_part, &data[i]);
		i++;
	}
	while (i--)
